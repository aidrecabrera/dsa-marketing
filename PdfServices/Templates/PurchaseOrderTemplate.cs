...
public class PurchaseOrderTemplate
{
            ...
            container.Page(page =>
            {
                ...
                page.Content().Column(c =>
                {
                    ...
                    c.Item().Border(0.1f).Height(250).MaxHeight(250).Table(itemList =>
                    {
                        ...
                        {
                            totalCost += Item(row, itemList, i.UnitName, i.Particulars, (int)i.Quantity!,
                                (float)i.Cost!);
                            row++;
                        }
                    ...
                    c.Item().Border(0.1f).Padding(5).Table(transactionInformation =>
                    {
                        ...
                        transactionInformation.Cell().ColumnSpan(5)
                            .Text(humanizedCost.ToWords().Transform(To.UpperCase))
                            .FontSize(9);
    ...
    public float Item(uint i, TableDescriptor itemList, string? itemName, string? description, int quantity,
        float unitCost,
        bool underlineLastCell = false)
    {
        ...
        if (underlineLastCell)
            itemList.Cell().Row(i).Column(7).ColumnSpan(1).Element(ItemFormat).AlignRight().PaddingLeft(10)
                .PaddingRight(10).Text((quantity * unitCost).ToString("C", philippinesCulture)).FontSize(10)
                .Underline();
        else
            itemList.Cell().Row(i).Column(7).ColumnSpan(1).Element(ItemFormat).AlignRight().PaddingLeft(10)
                .PaddingRight(10).Text((quantity * unitCost).ToString("C", philippinesCulture)).FontSize(10);
